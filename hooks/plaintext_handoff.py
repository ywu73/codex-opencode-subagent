...
def state_root(value: Optional[str]) -> pathlib.Path:
    ...
    override = os.environ.get("CODEX_OPENCODE_HANDOFF_DIR")
    if override:
        return pathlib.Path(override).expanduser().resolve()
    if os.name == "nt":
        local_app_data = os.environ.get("LOCALAPPDATA")
        if local_app_data:
            return pathlib.Path(local_app_data) / "Codex" / "opencode-plaintext-handoff"
    return pathlib.Path(os.environ.get("XDG_STATE_HOME", pathlib.Path.home() / ".local" / "state")) / "codex" / "opencode-plaintext-handoff"
...
def stage_locked(root: pathlib.Path, ttl_seconds: int, assignment: str, agent_type: str) -> Tuple[dict, pathlib.Path]:
    ...
    if pending.exists():
        try:
            ...
            fail("The existing focused OpenCode handoff is malformed. Refusing to replace it.", 9)
...
def run_target_hook_locked(root: pathlib.Path, hook_input: dict) -> None:
    ...
    try:
        ...
        quarantine_claim(claimed, agent_id)
        fail("The pending focused OpenCode handoff is malformed or has an invalid schema.", 5)
