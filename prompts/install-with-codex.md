...
(opencode_worker_ds_flash_v4 and opencode_worker_ds_pro_v4) from this
...
3. Install exactly these two agent files from the repository checkout:
  <codex-home>/agents/opencode-worker-ds-flash-v4.toml (opencode_worker_ds_flash_v4,
   deepseek-v4-flash),
  <codex-home>/agents/opencode-worker-ds-pro-v4.toml (opencode_worker_ds_pro_v4,
...
8. Parse each installed agent file with a real TOML parser. Confirm that the
   two files name opencode_worker_ds_flash_v4 and opencode_worker_ds_pro_v4 respectively, select
