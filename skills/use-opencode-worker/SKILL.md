...
- Read `config/opencode-worker-routing.json` from the current workspace before
  ...
  the exact default `opencode_worker_ds_flash_v4`; do not infer or select another profile.
...
| --- | --- | --- |
| `opencode_worker_ds_flash_v4` | `deepseek-v4-flash` | Available default; bounded reading and extraction |
| `opencode_worker_ds_pro_v4` | `deepseek-v4-pro` | Available; deeper reasoning |
...
2. Pipe the assignment through stdin to the installed handoff script in
   ...
   - Windows: `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "<codex-home>\hooks\codex-opencode-subagent\plaintext-handoff.ps1" -Mode stage -AgentType opencode_worker_ds_flash_v4`
   - macOS/Linux: `python3 "<codex-home>/hooks/codex-opencode-subagent/plaintext_handoff.py" --mode stage --agent-type opencode_worker_ds_flash_v4`
   (Use the chosen family type, e.g. `opencode_worker_ds_pro_v4`, in both the stage
