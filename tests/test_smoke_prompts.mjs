...
for (const promptPath of promptPaths) {
  test(`${promptPath} keeps one selected agent type through stage, spawn, and oracle`, () => {
    ...
    assert.doesNotMatch(prompt, /Spawn the exact agent type opencode_worker/);
    assert.doesNotMatch(prompt, /distinct opencode_worker child/);
    assert.doesNotMatch(prompt, /spawned opencode_worker child/);
    assert.doesNotMatch(prompt, /agent_type opencode_worker/);
    assert.doesNotMatch(prompt, /distinct native `opencode_worker` child/);
  });
}
