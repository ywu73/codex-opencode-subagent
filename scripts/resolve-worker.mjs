...
const defaultConfigPath = path.resolve(
  scriptDirectory,
  "../config/opencode-worker-routing.json",
);
...
function resolveProfileName(config, value) {
  if (typeof value !== "string" || !value.trim()) {
    fail("A worker selector must be a non-empty string.");
  }
  if (Object.hasOwn(config.workers, value)) {
    return value;
  }
  const alias = config.aliases?.[value];
  if (alias && Object.hasOwn(config.workers, alias)) {
    return alias;
  }
  const matchingProfile = Object.entries(config.workers).find(
    ([, worker]) => worker.agent_type === value,
  );
  if (matchingProfile) {
    return matchingProfile[0];
  }
  fail(`Unknown worker profile, alias, or agent type: ${value}`);
}

function assertWorkerProfile(profileName, worker) {
  if (!isObject(worker)) {
    fail(`Worker profile ${profileName} must be an object.`);
  }
  for (const field of [
    "agent_type",
    "model",
    ...
    if (typeof worker[field] !== "string" || !worker[field]) {
  ...
  for (const field of ["strengths", "avoid"]) {
    if (
      ...
      worker[field].some((item) => typeof item !== "string" || !item)
    ) {
      fail(`Worker profile ${profileName} has an invalid ${field} list.`);
    }
  }
  if (!isObject(worker.validation)) {
    fail(`Worker profile ${profileName} must declare validation status.`);
  }
}

export function validateRoutingConfig(config) {
  if (!isObject(config) || config.version !== 1) {
    fail("Routing config must be an object with version 1.");
  }
  if (!isObject(config.workers) || Object.keys(config.workers).length === 0) {
    fail("Routing config must define at least one worker profile.");
  }
  if (!isObject(config.aliases) || !isObject(config.task_profiles)) {
    fail("Routing config must define aliases and task_profiles objects.");
  }
  if (typeof config.default_profile !== "string") {
    fail("Routing config must define default_profile.");
  }
  ...
  for (const [profileName, worker] of Object.entries(config.workers)) {
    ...
    if (agentTypes.has(worker.agent_type)) {
      fail(`Agent type is duplicated: ${worker.agent_type}`);
    }
    if (models.has(worker.model)) {
      fail(`Model is duplicated: ${worker.model}`);
    }
  ...
  for (const [alias, profile] of Object.entries(config.aliases)) {
    if (typeof alias !== "string" || !alias || typeof profile !== "string") {
      fail("Routing config contains an invalid alias.");
    }
    resolveProfileName(config, profile);
  }
  for (const [task, profile] of Object.entries(config.task_profiles)) {
    if (typeof task !== "string" || !task || typeof profile !== "string") {
      fail("Routing config contains an invalid task profile.");
    }
    resolveProfileName(config, profile);
  }
  return config;
...
export function loadRoutingConfig(configPath = defaultConfigPath) {
  ...
  return validateRoutingConfig(config);
}

function findProfileBy(config, field, value) {
  const match = Object.entries(config.workers).find(
    ([, worker]) => worker[field] === value,
  );
  if (!match) {
    fail(`No worker profile matches ${field}=${value}.`);
  }
  return match[0];
}
...
export function resolveSelection(config, request = {}) {
  ...
  const selectors = ["profile", "worker", "agent_type", "model", "task"].filter(
    (field) => request[field] !== undefined,
  );
  if (selectors.length > 1) {
    fail(`Choose only one selector; received: ${selectors.join(", ")}.`);
  }
  ...
  if (selector === "profile" || selector === "worker") {
    ...
    if (!taskProfile) {
      fail(`Unknown task profile: ${request.task}`);
    }
...
function printHelp() {
  process.stdout.write(
    [
      ...
      "  --config <path>",
      "",
      "The command prints the resolved profile as JSON. The parent must stage and",
      "spawn the returned agent_type exactly; it must not silently fall back.",
    ].join("\n") + "\n",
  );
}
...
export function main(argv = process.argv.slice(2)) {
  try {
    ...
    process.stdout.write(`${JSON.stringify(resolveSelection(config, request), null, 2)}\n`);
  } catch (error) {
    process.stderr.write(`worker selection failed: ${error.message}\n`);
    process.exitCode = 2;
  }
}
