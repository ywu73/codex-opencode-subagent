...
| --- | --- | --- |
| `opencode_worker_ds_flash_v4`（OpenCode Worker DS Flash v4） | `deepseek-v4-flash`（默认） | enabled；Responses 探针通过 |
| `opencode_worker_ds_pro_v4`（OpenCode Worker DS Pro v4） | `deepseek-v4-pro` | enabled；Responses 探针通过 |
...
- `<codex-home>/agents/opencode-worker-ds-flash-v4.toml` 和 `opencode-worker-ds-pro-v4.toml`
...
- Codex 暴露独立原生 child，agent type 为你选定的 `opencode_worker_ds_flash_v4` 或
