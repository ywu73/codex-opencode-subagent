...
它直接复用 `codex-deepseek-subagent` 的架构：`opencode_worker` 是 Codex 原生
child，Agent TOML 自带 `model_provider`、`model`、`base_url`、`wire_api` 和
...
- 一条 `SubagentStart` Hook，matcher 为 `^opencode_worker$`
...
1. 在 Codex 输入 `/hooks`，确认它只匹配 `opencode_worker`，命令指向刚安装的
   `plaintext_handoff.py`，然后信任。
2. 新开一个 Codex 任务。
...
- Codex 暴露独立原生 child，agent type 为 `opencode_worker`；
...
- `agents/opencode-worker.toml`：child provider 只存在于这个独立 Agent 文件。
